...
const Tabs = ({ tabs, activeTabId, onTabClick, className }: TabsProps) => (
  <div className={cn('inline-flex flex-nowrap items-center border-b border-outline-variant', className)}>
    {tabs.map(tab => (
      <button
        key={tab.id}
        className='outline-none bg-transparent block cursor-pointer text-on-surface-var label-l border-solid border-b-2 border-transparent data-[active="true"]:border-primary data-[active="true"]:text-on-surface px-4 pt-3.5 pb-3 transition-colors min-w-32'
